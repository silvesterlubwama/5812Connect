...
async def analytics_overview(months: int = 12, current_user: dict = Depends(get_current_user)):
    ...
    members = await db.members.count_documents({**await get_campus_filter(current_user)})
    active = await db.members.count_documents({**await get_campus_filter(current_user), "status": "active"})
