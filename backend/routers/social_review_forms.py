...
async def _append_timeline_note(child_id: str, kind: str, doc: dict, user: dict, review_id: str):
    """Append a one-line summary to the active case's timeline notes — non-fatal."""
    try:
        ...
        if case:
            summary = {
                "school_progress": f"School progress review filed — overall: {doc.get('overall_assessment') or 'n/a'}",
                "welfare_visit": f"Home/welfare visit logged — overall: {doc.get('overall_assessment') or 'n/a'}",
