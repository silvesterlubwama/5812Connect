...
async def upload_item_photo(
    ...
    try:
        ...
        file_url = await save_upload("shipment-items", unique, data, file.content_type)
    except Exception as e:
        logger.warning(f"Item photo upload failed: {e}")
...
# iter 263 — Printable PDF endpoints (manifest.pdf, schematic.pdf,
# commercial-invoice.pdf) moved to `pdfs.py`. Routes are unchanged; they
# just live in a dedicated submodule now to keep this file focused on
# item CRUD + AI scan flows.
# iter 263 — Printable PDF endpoints (manifest.pdf, schematic.pdf,
# commercial-invoice.pdf) moved to `pdfs.py`. Routes are unchanged;
# they just live in a dedicated submodule now to keep this file
# focused on item CRUD + AI scan flows.
