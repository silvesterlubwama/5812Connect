...
export default function Layout() {
  ...
  useEffect(() => {
    const loadCampuses = async () => {
      try {
        ...
        const locs = res.data || [];
        setCampuses(locs);
        // Auto-migrate legacy "All my campuses" state — no such option any more.
        // If active campus is blank or invalid, snap to the user's home campus.
        let activeId = activeCampus || user?.location_id;
        if (activeId && !locs.find(l => l.id === activeId)) activeId = user?.location_id || (locs[0] && locs[0].id) || '';
        if (!activeCampus && activeId) {
          setActiveCampus(activeId);
          localStorage.setItem('5812_active_campus', activeId);
          api.put('/user/active-campus', { campus_id: activeId }).catch(() => {});
        }
        if (activeId) {
  ...
  const handleCampusChange = async (val) => {
    // No blank / "all" option any more — every choice is a real campus id.
    if (!val || val === '__all__') return;
    setActiveCampus(val);
    localStorage.setItem('5812_active_campus', val);
    try {
      await api.put('/user/active-campus', { campus_id: val });
