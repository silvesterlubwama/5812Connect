...
import { CreditCard, UserCheck, Eye, EyeOff } from 'lucide-react';
...
export default function KioskPage() {
  ...
  const [view, setView] = useState('home'); // 'home' | 'visitor'
        ...
        <Card className="w-full max-w-md shadow rounded-xl">
          ...
          <CardContent>
            <form onSubmit={handleVisitorCheckin} className="space-y-4">
              ...
              <Button type="submit" className="w-full h-14 text-lg">Check In as Visitor</Button>
        ...
        <CardContent className="space-y-5">
          ...
          <div className="p-3 rounded-lg border border-dashed border-primary/30 bg-primary/5">
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Unlinked devices can still login, but venue will not be statically locked.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Button
                variant="outline"
                className="h-16 gap-3 justify-start px-4 text-left"
                onClick={() => toast.info('ID/QR scanner would open here')}
              >
          ...
          <form onSubmit={handleStaffLogin} className="space-y-4">
            ...
            <Button type="submit" className="w-full h-14 text-lg" disabled={loading}>
              {loading ? 'Signing in...' : 'Sign In to Kiosk'}
            </Button>
          </form>
