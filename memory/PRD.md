...
- **Chart of Accounts fully editable — root cause fix.** `AccountingPage.jsx` was calling `/api/accounting/*`, which was deliberately dropped in the iter 246 finance reset but the UI never got re-wired — returned 404 for every account and Opening Balance had no rows to attach to. Repointed to `/api/finance/chart-of-accounts` for list/create/edit/delete and `/api/finance/journal` for entries; also normalised entry-row field names (`id → number`, `description → narration`, `source → journal_code`, `total → total_debit`, `reversed → is_reversed`) so the existing UI keeps rendering. Opening Balance dialog now targets a real account and posts the balanced JE against `3000 Opening Balance Equity`. Verified via curl: PUT `/finance/chart-of-accounts/{id}` returns the updated doc; opening balance for 5000 UGX posts as `Dr Petty Cash 5000 / Cr Opening Balance Equity 5000`.
- **New tasks now visibly save.** `TasksPage.addCard` was doing an optimistic-only update; when WS `card_created` for the creator was excluded, and the local state flushed before render, the card silently disappeared until refresh. Added a 300 ms delayed `fetchBoardDetail()` + a success toast so the card always shows up in the column even if the optimistic push loses a race.
- **Badge — long names no longer hide behind the QR.** `UnifiedBadge.jsx` body is now a two-column layout: LEFT stacks info at the top and QR pinned to the bottom (`flexDirection: column; justifyContent: space-between`); RIGHT is the photo, sized taller (148 px on the normal badge, 108 px on small) so its bottom edge sits flush with the QR bottom. Info block reads top-down without the QR/photo shoving it off — matches the request "move user details up level with current, then push QR down to align with photo's bottom line".
- **Consumable manual tracking sheets** — `POST /api/resources/{id}/tracking-sheet` returns a printable HTML worksheet with a 31-row day-by-day dispensing table, initials column, and an auto-generated serving-conversion cheat-sheet (Ugandan cup ≈ 500 g rice, ≈ 333 g posho, ≈ 400 g beans, ≈ 250 g sugar, ≈ 200 g salt, ≈ 250 mL oil, soap bar splittable ×4). `POST /api/resources/{id}/tracking-sheet/upload` lets a coordinator drop the total servings for a month + serving unit; we look up per-base conversion, convert to the base unit, refuse over-consumption, insert a `consumable_sheets` audit row AND an `out` movement so on-hand stays in sync. Ugandan defaults are opinionated but admins can override via `resource.serving_conversions`.
  - Frontend: on each consumable card the "Print Sheet" button opens the HTML in a new tab (auth via bearer); "Upload Sheet" opens a dialog with month + total-servings + serving-unit picker driven by the resource's own conversions.
- **Auto-issue tickets to children (not general public)** — `POST /api/events/{event_id}/issue-tickets` accepts `{audience: 'children'|'members'|'custom', member_ids?, location_id?, tier_id?, custom_label?, note?}`. Creates one `public_bookings` row per recipient with `auto_issued=true`, dedupes on `(event_id, member_id)`, respects tier capacity, bumps `registered` + tier `sold`. Manager+ only. Companion `GET /api/events/{id}/auto-issued` lists who already has a ticket. Custom label prints on the ticket for per-event customisation.
- **Deployment-safe (`CI=true yarn build`) still passes** after all changes.
